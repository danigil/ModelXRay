...
def _preprocessed_image_pipeline(preprocessed_image_lineage: PreprocessedImageLineage):
    cover_data = fetch_cover_data_step(cover_data_config=preprocessed_image_lineage.cover_data_config)

    if preprocessed_image_lineage.embed_payload_config != ret_na_val():
        stego_data = embed_payload_into_cover_data_step(cover_data=cover_data, embed_payload_config=preprocessed_image_lineage.embed_payload_config)
        data = stego_data
    else:
        data = cover_data

    image_rep = create_image_representation_step(data=data, image_rep_config=preprocessed_image_lineage.image_rep_config)
    preprocessed_image = image_preprocessing_step(image=image_rep, image_preprocess_config=preprocessed_image_lineage.image_preprocess_config)
